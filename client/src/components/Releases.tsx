...
export default function Releases() {
  ...
  return (
    <div className="bg-black/60 backdrop-blur-md rounded-lg border border-primary/30 p-4 sm:p-6">
      <motion.h2
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="text-2xl sm:text-3xl md:text-4xl font-display text-primary glow-cyan mb-4 sm:mb-6"
      >
        {t('releases.title')}
      </motion.h2>
        ...
        <div className="space-y-4">
          {sortedReleases.map((release, index) => (
            <motion.div
              ...
              className="group flex gap-3 sm:gap-4 bg-card/30 border border-border hover:border-primary/50 rounded-lg p-3 sm:p-4 transition-all"
            >
              {/* Capa */}
              ...
              <div className="flex-1 flex flex-col justify-between min-w-0">
                <div>
                  <h3 className="text-base sm:text-lg md:text-xl font-display text-primary glow-cyan truncate">
                    {release.title}
                  </h3>
                ...
                <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-2">
                  {release.links?.spotify && (
                    <a
                      ...
                      className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-primary/10 border border-primary/30 hover:bg-primary/20 hover:border-primary rounded text-xs font-medium text-primary transition-all"
                    >
